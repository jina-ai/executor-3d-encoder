"""ref : https://github.com/ma-xu/pointMLP-pytorch/blob/main/classification_ModelNet40/models/pointmlp.py
"""
...
from ..pointconv import farthest_point_sample, index_points, knn_point, query_ball_point
...
class LocalGrouper(nn.Module):
    ...
    def forward(self, xyz, points):
        ...
        if self.normalize is not None:
            ...
            diff = (grouped_points - mean).reshape(B, -1)
            std = (
                torch.std(diff, dim=-1, keepdim=True)
                .unsqueeze(dim=-1)
                .unsqueeze(dim=-1)
            )
...
class Model(nn.Module):
    def __init__(
        ...
        **kwargs,
        ...
        self.get_embedding = nn.Linear(last_channel, self.embed_dim)
